...
path_Background = cv2.imread('Resources/background.png')
...
frame_skip = 2  # Adjust the frame skip count
frame_count = 0
...
while True:
    ...
    if live_faces:
        for encodeFace, faceloc in zip(face_encodings, live_faces):
            ...
            # Only load if confidence is in range
            if matches[matchIndex] and 0.1 <= facedis[matchIndex] <= 0.45:  
